    ...
    public function mpesaC2BConfirm()
    {
        ...
        if ($postData != null) {
            ...
            if ($prefix == "MDR") {
                ...
                Log::info("Forwarding transaction to the mdarasa publisher endpoint >> " . json_encode($payload));

                $returned = $this->callServiceEndpoint($payload, self::MDARASA_PUBLISHER_ENDPOINT);

                if ($returned?->success) {

                    Log::info("Read successful Response from Mdarasa Publisher");
                    return;

                } else {

                    Log::info("Failed to forward the transaction to the MDarasa Publisher endpoint");
